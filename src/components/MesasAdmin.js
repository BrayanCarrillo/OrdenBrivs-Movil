...
const MesasAdmin = () => {
    ...
    const handleAgregarEditarMesa = async () => {
        if (nombreMesa.trim() === '') {
            Alert.alert("Por favor ingresa el número de la mesa.");
            return;
        }

        try {
            if (editingMesaId) {
                console.log(`Editing mesa ID: ${editingMesaId} with new ID: ${parseInt(nombreMesa.trim())}`);
                await axios.put(`https://evolving-man-proud.ngrok-free.app/api/mesas/${editingMesaId}`, {
                    mesaID: parseInt(nombreMesa.trim()), // Asegurar que mesaID es un entero
                    activate: true // Valor por defecto
                });
                const updatedMesas = mesas.map(mesa => {
                    if (mesa.id === editingMesaId) {
                        return { ...mesa, id: nombreMesa.trim(), nombre: nombreMesa.trim() };
                    }
                    return mesa;
                });
                setMesas(updatedMesas);
                setEditingMesaId(null);
                setNombreMesa(''); // Limpiar el número de la mesa después de la edición
                ...
                setNombreMesa(''); // Limpiar el número de la mesa después de agregarla
    ...
    const handleBorrarMesa = async (id) => {
        try {
            console.log(`Deleting mesa with ID: ${id}`);
